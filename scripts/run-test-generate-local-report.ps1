param (
    ...
    [string]$Headless = ""   # Empty = use .env value; pass 'true' or 'false' to override
)

Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "K6 Hybrid Performance Test Runner" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan

# Step 0: Use cached token if < 90 minutes old, otherwise re-login
...
if (Test-Path $tokenPath) {
    ...
    if ($tokenAge.TotalMinutes -lt 60) {
        Write-Host "Cached token is $([math]::Round($tokenAge.TotalMinutes, 1)) min old - skipping login." -ForegroundColor Green
        $needsLogin = $false
    }
    else {
        Write-Host "Token expired ($([math]::Round($tokenAge.TotalMinutes, 1)) min old) - re-logging in." -ForegroundColor Yellow
    }
}

if ($needsLogin) {
    try {
        npx playwright test ./setup/getTokenViaLogin.spec.js --retries=1
    ...
    Write-Host "Bearer token refreshed successfully.`n" -ForegroundColor Green
}

# Read captured token from .token file
$bearerToken = (Get-Content ".token" -Raw).Trim()
if ([string]::IsNullOrWhiteSpace($bearerToken)) {
    Write-Error "Bearer token is empty after Playwright run. Aborting."
    exit 1
}
...
# Step 2: Apply -Headless override if explicitly passed (takes priority over .env)
if ($Headless -ne "") {
    $env:HEADLESS = $Headless
    Write-Host "  HEADLESS overridden by parameter: $Headless" -ForegroundColor Gray
}

# Step 3: Validate testEnv (default to 'qa' if not set)
$testEnv = $env:testEnv
if (-not $testEnv) {
    $testEnv = "qa"
    Write-Host "`ntestEnv not set. Using default: $testEnv" -ForegroundColor Yellow
}
else {
